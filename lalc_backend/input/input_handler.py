...
if IS_WINDOWS:
    ...
    from input.mac_adapter import (
        ...
        close_limbus_window,
        take_screenshot,
        _get_logical_client_rect,
        background_click,
        foreground_click,
        background_long_press,
        foreground_long_press,
        background_key_press,
        foreground_key_press,
        background_swipe,
        foreground_swipe,
    )
...
def _safe_block_input(enabled: bool):
    if IS_WINDOWS:
        ctypes.windll.user32.BlockInput(enabled)


def _safe_set_process_dpi_awareness():
    if IS_WINDOWS:
        ctypes.windll.shcore.SetProcessDpiAwareness(2)


def _safe_set_cursor_pos(x, y):
    if IS_WINDOWS:
...
class ForegroundInputState(InputState):
    ...
    def click(self, hwnd, x, y):
        # 在执行点击前阻止用户输入
        _safe_block_input(True)
        try:
            return foreground_click(hwnd, x, y)
        finally:
            # 确保无论如何都会解除阻止
            _safe_block_input(False)
...
class _Input:
    ...
    def __init__(self):
        ...
        _safe_set_process_dpi_awareness()

        screen_width = _safe_screen_width()
    ...
    def reset_mouse_position(self):
        _safe_set_cursor_pos(*self._mouse_reset_position)
