...
def encrypt_cdk(text: str) -> str:
    ...
    # 非 Windows 或不可用 win32crypt 时，使用可逆 Base64 包装
    if win32crypt is None:
        return "PLAIN:" + base64.b64encode(data).decode('utf-8')
...
def decrypt_cdk(encrypted_b64: str) -> str:
    ...
    try:
        # 非 Windows 或不可用 win32crypt 时，支持 PLAIN: 前缀解密
        if win32crypt is None:
            if encrypted_b64.startswith("PLAIN:"):
                payload = encrypted_b64[len("PLAIN:"):]
